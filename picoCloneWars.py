...
def deepcopy(someArray):
    return([someArray[i] for i in range(len(someArray))])
...
class Simulator:
    ...
    def __init__(self, size, mutationRate=0, wait=1, numberColours=4, height=None, advantage=0, special=None):
        ...
        self.nextFitness = [0. for _ in range(self.population)]
        self.nextColour = [0 for _ in range(self.population)]
    ...
    def mutate(self, colour=None):
        ...
        cell = random.randint(0, (self.population-1))
        self.fitness[cell] += random.uniform((self.advantage-0.15), (self.advantage+0.15)) # originally normal
        if colour == None:
    ...
    def update(self):
        ...
        self.fitness = deepcopy(self.nextFitness)
        self.colour = deepcopy(self.nextColour)
